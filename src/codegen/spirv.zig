...
pub const DeclGen = struct {
        ...
        switch (ty.zigTypeTag(mod)) {
            .Int => {
                if (ty.isSignedInt(mod)) {
                    try self.genConstInt(result_ty_ref, result_id, val.toSignedInt(mod));
                } else {
                    try self.genConstInt(result_ty_ref, result_id, val.toUnsignedInt(mod));
                }
            },
            .Bool => {
        ...
        switch (ty.zigTypeTag(mod)) {
            ...
            .Fn => switch (repr) {
                .direct => {
                    ...
                    const param_types = try self.spv.arena.alloc(SpvType.Ref, fn_info.param_types.len);
                    for (param_types, fn_info.param_types) |*param, zig_ty| {
                        param.* = try self.resolveType(zig_ty.toType(), .direct);
                    }

                    const return_type = try self.resolveType(fn_info.return_type.toType(), .direct);
    ...
    fn airSliceElemVal(self: *DeclGen, inst: Air.Inst.Index) !?IdRef {
        ...
        const ptr_ty_id = try self.resolveTypeId(slice_ty.slicePtrFieldType(mod));
